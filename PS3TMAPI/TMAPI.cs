...
public class TMAPI
{
    ...
    {
        public static string Usage, Status, ConsoleName;
    ...
    public void GetProcessList(out uint[] pids)
    {
        PS3TMAPI.GetProcessList(Target, out pids);
    }

    public void GetModuleList(uint pid, out uint[] modules)
    {
        PS3TMAPI.GetModuleList(Target, pid, out modules);
    }
    ...
    public bool AttachProcess()
    {
        ...
        {
            Parameters.ProcessID = Convert.ToUInt32(Parameters.ProcessIDs[0]);
    ...
    public void SetMemory(uint pid, uint Address, byte[] Bytes)
    {
        PS3TMAPI.ProcessSetMemory(Target, PS3TMAPI.UnitType.PPU, pid, 0, Address, Bytes);
    }
    ...
    public void GetMemory(uint pid, uint Address, byte[] Bytes)
    {
        PS3TMAPI.ProcessGetMemory(Target, PS3TMAPI.UnitType.PPU, pid, 0, Address, ref Bytes);
    }
    ...
    public string GetModulePath(uint pid, uint moduleId)
    {
        if (moduleId != 0)
        {
            PS3TMAPI.ModuleInfo moduleInfo;
            PS3TMAPI.GetModuleInfo(Target, pid, moduleId, out moduleInfo);
            return moduleInfo.Hdr.ELFName;
        }
        return "";
    }
    ...
    public PS3TMAPI.ProcessInfo GetProcessInfo(uint processID)
    {
        PS3TMAPI.ProcessInfo processInfo;
        PS3TMAPI.GetProcessInfo(Target, processID, out processInfo);
        return processInfo;
    ...
    public void ProcessStop(uint pid)
    {
        PS3TMAPI.ProcessStop(Target, pid);
    }

    public void ProcessContinue(uint pid)
    {
        PS3TMAPI.ProcessContinue(Target, pid);
    }

    public void ProcessKill(uint pid)
    {
        PS3TMAPI.ProcessKill(Target, pid);
    }
    ...
    public uint GetModuleEndAddress(uint moduleId)
    {
        if (Parameters.ProcessIDs.Length > 0)
        {
            PS3TMAPI.ModuleInfo moduleInfo;
    ...
    public ulong GetModuleSize(uint moduleId)
    {
        if (Parameters.ProcessIDs.Length > 0)
        {
            ...
            for (int i = 0; i < moduleInfo.Segments.Length; i++)
                size += moduleInfo.Segments[i].MemSize;
            return size;
        }
        return 0x0;
    }
    ...
    public string GetCurrentGame()
    {
        ...
        PS3TMAPI.GetProcessInfo(0, Parameters.ProcessID, out processInfo);
    ...
    public class Extension
    {
        ...
        private byte[] GetBytes(uint pid, uint offset, int length)
        {
            byte[] buffer = new byte[length];
            Target.GetMemory(pid, offset, buffer);
            return buffer;
        }
        ...
        private void SetBytes(uint pid, uint offset, byte[] bytes)
        {
            Target.SetMemory(pid, offset, bytes);
        }
        ...
        public byte[] ReadBytes(uint pid, uint offset, int length)
        {
            byte[] buffer = GetBytes(pid, offset, length);
            return buffer;
        }
        ...
        public void WriteBytes(uint pid, uint offset, byte[] input)
        {
            byte[] buff = input;
            SetBytes(pid, offset, buff);
        }
